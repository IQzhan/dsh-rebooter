...
check('client injects the slots package', pkg.dsh?.client?.inject, ['@deepseek-ai/dsh-client-ui-slots'])
...
check('client occupies sidebar.footer.action', slot?.options?.name, 'sidebar.footer.action')
check('client uses its own id so it does not replace Cordis', slot?.options?.id, 'dsh-rebooter')
