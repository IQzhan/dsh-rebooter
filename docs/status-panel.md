...
- **Window shell** = OS-specific thin host that loads the local URL. Generated
  under the **plugin package** (`package/panel/…`) with a Desktop shortcut
  named **DSH Server**.
...
| DSH web | Buttons |
| --- | --- |
| **Stopped** | Start · Update · Update & start |
| **Running** | Stop · Restart · Update & stop · Update & restart · Open UI |
...
| Update & start | `update-restart` |
| Stop | `stop` |
| Restart | `restart` |
| Update & stop | `update-stop` |
| Update & restart | `update-restart` |
| Open UI | `open` (new: browser or bound app) |

In-app sidebar menu stays **menu-only** (`stop` / `restart` / `update-*`):
DSH is already running. Opening the panel for progress is automatic when those
run.
...
- **Open UI** opens the bound target, or the browser at `web.url` / canonical URL.
- Gear (**bind app**): pick an executable; then the gear becomes **Clear**.
- Checkbox **Open UI when DSH starts** ↔ `panel.json.autoOpen` (default off).
- `start` / `restart` / `update-restart` call `openUrl` / bound app **only**
  when `autoOpen` is true (or CLI `--open` is passed explicitly).
...
2. Write a Desktop shortcut named **`DSH Server`**. The shortcut is the only file that records where the package is installed, and Host mount rewrites it. Its icon is `panel/dsh-server.ico`, never the `.vbs`.
...
| OS | Shell |
| --- | --- |
| Every desktop | One frameless host (`decorations: false`). The page draws the title bar, minimize, and close. Dragging that bar moves the window through the host position API. No OS caption buttons, and no per-OS window code in this plugin. |
...
- Client: unchanged menu actions; no second start button.
...
- Package: `cli.cjs` exposes `panel` / `update` / `open`; built client still menu-only.
