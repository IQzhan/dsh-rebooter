...
为 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 提供一个轻量插件，用来**开启服务 / 关闭服务 / 重启服务 / 更新插件 / 更新 DSH / 打开界面**。默认打开的页面是独立窗口，不依赖浏览器。页面窗口操作栏的电源按钮弹出**插件**生命周期菜单；桌面一个 **DSH Server** 状态面板入口。更新插件会先关掉 DSH，再升级当前 profile 里的**全部插件**。更新 DSH 本体是另一套面板 / CLI 动作。
...
**这两个窗口都只是展示壳。** 关掉或最小化，已经在跑的服务和监督进程不受影响。要停服务，用「关闭服务」或「更新插件并关闭服务」。窗口自己的关闭按钮只关窗口。

独立窗口靠系统里的网页组件：Windows 要有 WebView2，macOS 用自带的 WebKit，Linux 要有 WebKitGTK。没有这些组件，或窗口创建失败时，插件不会帮忙安装。这时 DSH Server 和 DSH 页面都改由系统默认程序打开，通常就是浏览器。浏览器里没有右上角按钮组，不能靠窗口空白处拖动，面板上的「浏览」也不能选文件，路径要自己填。开关、重启、更新插件、更新 DSH、语言和主题都还在。
...
未运行：开启服务；插件：更新插件 / 更新插件并开启服务；DSH：更新 DSH / 更新 DSH 并开启服务。勾选「开启服务时打开界面」后，下次开服务会一起打开页面。

![服务运行中](docs/images/panel-running.png)

运行中：关闭服务 / 重启服务；插件：更新插件并关闭服务 / 更新插件并重启服务；DSH：更新 DSH 并关闭服务 / 更新 DSH 并重启服务；打开界面。齿轮用来绑定别的程序；绑了之后，「打开界面」打开那个程序。

![页面右上角的电源菜单](docs/images/page-menu.png)

上图就是独立窗口，不是浏览器标签。鼠标移到右上角出现：电源、最小化、最大化、关闭。电源是四项**插件**动作（没有开启服务、没有更新 DSH）。关闭只关窗口。Windows 上拖顶部空白可用 Aero Snap；双击等于最大化/还原按钮。
...
| `update-restart` | 是 | 是 |
| `update-dsh` | 否 | 是（未启动时） |
| `update-dsh-stop` | 否 | 是（已启动时） |
| `update-dsh-restart` | 否 | 是 |
| `open` | 否 | 是（已启动时） |
| `panel` | — | 打开 **DSH Server** |

这四项从页面窗口操作栏的电源按钮弹出。点选后请求 `/api/dsh-rebooter/action`。菜单里没有开启服务，也没有更新 DSH。
...
| 文件 | 作用 |
| --- | --- |
| `dsh-rebooter-core.js` | 纯策略：动作名、端口、可用性矩阵、DSH 分类与配方计划 |
| `dsh-rebooter-runtime.js` | Node IO：状态目录、监督进程、任务、桌面入口、DSH 配方执行 |
...
**8 个套件、260 条断言**：策略核心 · 运行时 · 面板 · 适配层 · 包 · 菜单 · 文档双语同步 · 可移植性守卫。
